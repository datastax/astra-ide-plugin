...
class UpsertDocumentsAction(var editor: Editor, val cBoxes: ToolbarComboBoxes,var state: Long=0L,text: String = message("collection.editor.upsert.title")):
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        if (jsonObject != null && jsonObject.isValid){
            ...
            val responseList = mutableListOf<Response<Unit>>()
            jsonObject?.children?.forEach { doc ->
                val docId = doc.children[0].text.trim('"')
                val jsonDoc = doc.children[1].text.trim()
                launch {
                    val response = AstraClient.documentApiForDatabase(selected.database).replaceDoc(
                        randomUUID(),
                        AstraClient.accessToken,
                        selected.keyspace,
                        selected.collection,
                        docId,
                        jsonDoc.toRequestBody("text/plain".toMediaTypeOrNull()) as RequestBody,
                    )
                    if (!response.isSuccessful) {
                        failedResponses.map { docId to response }
                    }
                }

            }
            if (failedResponses.isNotEmpty()) {
                failedDocUpsertNotification(failedResponses)
            }
